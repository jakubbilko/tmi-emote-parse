      ...
      .then((body) => {
        try {
          ...
          if (loadedAssets[channel].allLoaded) {
            loadedAssets[channel].emotes =
              loadedAssets[channel].emotes.sort(compareLength);
                  ...
                  try {
                    if (body.status == undefined && body.status != 404) {
                      ...
                      exports.events.emit("error", {
                        channel: channel,
                        error:
                          "Failed to load 7TV global emotes for " + channel,
                      });
    ...
    .then((body) => {
      try {
        ...
        if (loadedAssets[channel].badgesLoaded.indexOf(false) == 2) {
          ...
          if (
            loadedAssets[channel].badgesLoaded[2] == true &&
            loadedAssets[channel].loaded[6] == true
          ) {
            exports.events.emit("loaded", {
              channel: channel,
            });
...
function checkLoaded(channel) {
  if (loadedAssets[channel].loaded[6]) {
    ...
    loadedAssets[channel].loaded[0] &&
    loadedAssets[channel].loaded[1] &&
    loadedAssets[channel].loaded[2] &&
    loadedAssets[channel].loaded[3] &&
    loadedAssets[channel].loaded[4] &&
    loadedAssets[channel].loaded[5] &&
    !loadedAssets[channel].loaded[6]
  ) {
    return 1;
  } else if (
    !loadedAssets[channel].loaded[0] ||
...
function checkLoadedAll(channel, type, extra, value, args) {
  ...
  if (
    args[type][extra] == true &&
    loadedAssets[channel].loaded[type][extra] == false &&
    value == true
  ) {
...
String.prototype.replaceAt = function (start, end, replacement) {
  ...
  return (
    this.substring(0, start) + emRep + this.substring(end + 1, this.length)
  );
};
...
function replaceBTTVAll(msg, channel) {
  ...
  loadedAssets[channel].emotes.forEach((ele) => {
    ...
    if (ele.type == "bttv") {
      ...
      msg = msg.replace(
        regex,
        `<img class="message-emote"src="https://cdn.betterttv.net/emote/${ele.id}/3x"/>`
      );
      ...
      var m = msg.match(regex);
      var poss =
        ele.urls[4] != undefined
          ? ele.urls[4]
          : ele.urls[2] != undefined
          ? ele.urls[2]
          : ele.urls[1];
      msg = msg.replace(
        regex,
        `<img class="message-emote"src="https:${poss}"/>`
      );
      ...
      var m = msg.match(regex);
      var poss =
        ele.urls[3][1] != undefined
          ? ele.urls[3][1]
          : ele.urls[2][1] != undefined
          ? ele.urls[2][1]
          : ele.urls[1][1];
...
function getBadges(tags, channel) {
  ...
  bg.forEach((ele, ind) => {
    var cur =
      loadedAssets[channel].badges[ele] != undefined
        ? loadedAssets[channel].badges[ele]
        : null;
    if (cur != null) {
      badges.push(cur);
    }
...
exports.getEmotes = function (message, tags, channel) {
  return getMessageEmotes(
    message,
    tags,
    channel.replace("#", "").trim().toLowerCase()
  );
};
